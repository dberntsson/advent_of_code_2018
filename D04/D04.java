...
public class D04 {
	...
	public static void main(String[] args) {
		final List<LogEntry> logEntries = 
				loadAndSortInput();
		final Map<String, List<String>> guardSchedule = 
				parseWorkLog(logEntries);
		final List<Guard> guardSleepsSchedule = 
				summarizeGuardSleepPatterns(guardSchedule);
		A1(guardSleepsSchedule);
		A2(guardSleepsSchedule);
	}

	private static Map<String, List<String>> parseWorkLog(final List<LogEntry> logEntries) {
		final Map<String, List<String>> guardSleeps = new HashMap<String, List<String>>();
		LogEntry currentGuard = null;
		Date startSleep = null;
		Date endSleep = null;
		for (Iterator<LogEntry> iter = logEntries.iterator(); iter.hasNext();) {
			final LogEntry logEntry = iter.next();
			final int eventType = logEntry.getEventType();
			if (eventType == LogEntry.BEGIN_SHIFT) {
				currentGuard = logEntry;
				if (guardSleeps.get(logEntry.getEvent()) == null) {
					guardSleeps.put(logEntry.getEvent(), null);
				}
				startSleep = null;
				endSleep = null;
			}
			if (eventType == LogEntry.FALLS_ASLEEP) {
				startSleep = logEntry.getEventDate();
			}
			if (eventType == LogEntry.WAKES_UP) {
				endSleep = logEntry.getEventDate();
			}
			if (currentGuard != null && startSleep != null && endSleep != null) {
				List<String> registeredSleepPeriods = guardSleeps.get(currentGuard.getEvent());
				if (registeredSleepPeriods == null) {
					registeredSleepPeriods = new ArrayList<String>();
				}
				registeredSleepPeriods.add(getSleepStringRepresentation(startSleep, endSleep));
				
				guardSleeps.put(currentGuard.getEvent(), registeredSleepPeriods);
			}
		}
		return guardSleeps;
	}

	private static void A1(List<Guard> guardSleepsSchedule) {
		Guard guardWhoSleepsMostInTotal = null;
		Integer maxValue = 0;
		for (Iterator<Guard> iter = guardSleepsSchedule.iterator(); iter.hasNext();) {
			final Guard guard = iter.next();
			if (guard.getTotalSleepHours() > maxValue) {
				guardWhoSleepsMostInTotal = guard;
				maxValue = guard.getTotalSleepHours();
			}
		}
		if (guardWhoSleepsMostInTotal != null) {
			System.out.println("A1:" + 
					guardWhoSleepsMostInTotal.getMaxSleepMinute() + " * " + 
					guardWhoSleepsMostInTotal.getId());
		}
	}
	
	private static void A2(List<Guard> guardSleepsSchedule) {
		Guard guardWhoSleepsMostSameMinute = null;
		Integer maxValue = 0;
		for (Iterator<Guard> iter = guardSleepsSchedule.iterator(); iter.hasNext();) {
			final Guard guard = iter.next();
			if (guard.getMaxSleepMinuteCount() > maxValue) {
				guardWhoSleepsMostSameMinute = guard;
				maxValue = guard.getMaxSleepMinuteCount();
			}
		}
		if (guardWhoSleepsMostSameMinute != null) {
			System.out.println("A2:" + 
					guardWhoSleepsMostSameMinute.getMaxSleepMinute() + " * " + 
					guardWhoSleepsMostSameMinute.getId());
		}
	}

	private static List<Guard> summarizeGuardSleepPatterns(Map<String, List<String>> guardSleeps) {
		List<Guard> summary = new ArrayList<Guard>();
		for (Iterator<Entry<String, List<String>>>  iterator = guardSleeps.entrySet().iterator(); iterator.hasNext();) {
			Entry<String, List<String>> entry = iterator.next();
			summary.add(new Guard(entry.getKey(), entry.getValue()));
		}
		return summary;
	}

	private static String getSleepStringRepresentation(Date start, Date end) {
		String sleepString = "";
		for (int i = 0; i < 60; i++) {
			if (i >= start.getMinutes() && i < end.getMinutes())
	...
	private static <T> List<LogEntry> loadAndSortInput() {
		final List<LogEntry> logEntries = new ArrayList<LogEntry>();;
		for (Iterator<String> iter = getInput().iterator(); iter.hasNext();) {
			final LogEntry logEntry = new LogEntry(iter.next());
